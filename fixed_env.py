...
class Environment:
    def __init__(self, all_cooked_time, all_cooked_bw, random_seed=RANDOM_SEED,
                 logfile_path='./', VIDEO_SIZE_FILE='./video_size_', VIDEO_TRACE='sports',
                 Debug=True):
        ...
        self.cdn_d_arrive_time = [] # dynamic arrive time
        self.gop_time_len = {}
        self.gop_flag = {}

        self.D_Trace = s2c.Trace(video_source=VIDEO_TRACE)
        self.D_Trace.upload_next_200_frame(cooked_bitrate=3, cdn_arrive_time=self.cdn_d_arrive_time)
        ...
        for bitrate in range(BITRATE_LEVELS):
            ...
            with open(self.video_size_file + str(bitrate)) as f:
                ...
                for line in f:
                    ...
                    # self.cdn_arrive_time[bitrate].append(int(float(line.split()[0])))
    ...
    def get_video_frame(self, quality, target_buffer, latency_limit, test_mod1=-1):
        ...
        while self.time >= self.D_Trace.source_side_time:
            next_upload_bitrate = self.D_Trace.decide_next_bitrate(test_mod1=test_mod1)
            self.D_Trace.upload_next_200_frame(cooked_bitrate=next_upload_bitrate, cdn_arrive_time=self.cdn_d_arrive_time)

        if self.time < self.cdn_d_arrive_time[self.video_chunk_counter] and not end_of_video:  # CDN can't get the frame
            self.delay_cnt += 1
            cdn_rebuf_time = self.cdn_d_arrive_time[self.video_chunk_counter] - self.time
            ...
            if not self.buffer_status:
                ...
                # not enough .let the client buffering
                    ...
                    rebuf = cdn_rebuf_time - (self.buffer_size / play_duration_weight)  #physical time
                ...
                self.latency = (self.cdn_d_arrive_time[self.newest_frame] - self.video_chunk_counter * FRAME_TIME_LEN) + self.buffer_size
                self.time = self.cdn_d_arrive_time[self.video_chunk_counter]
            else:
                rebuf = duration
                play_len = 0
                self.time = self.cdn_d_arrive_time[self.video_chunk_counter]
            ...
            return [self.time,  # physical time
                    ...
                    end_of_video,
                    self.delay_cnt]  # Is the end of video
        else:
            the_newst_frame = self.video_chunk_counter
            current_new = self.cdn_d_arrive_time[self.video_chunk_counter]
            while (current_new < self.time):
                if self.time >= self.D_Trace.source_side_time:

                    while self.time >= self.D_Trace.source_side_time:
                        next_upload_bitrate = self.D_Trace.decide_next_bitrate(test_mod1=test_mod1)
                        self.D_Trace.upload_next_200_frame(cooked_bitrate=next_upload_bitrate,
                                                           cdn_arrive_time=self.cdn_d_arrive_time)
                the_newst_frame += 1

                current_new = self.cdn_d_arrive_time[the_newst_frame]
        ...
        if self.buffer_status and not end_of_video:
            ...
            # calculate the play time , the real time ,the latency
            self.play_time_counter = int(self.play_time / FRAME_TIME_LEN)
            self.latency = (self.cdn_d_arrive_time[self.newest_frame] - self.video_chunk_counter * FRAME_TIME_LEN) + self.buffer_size # re-calculate this
            # Debug Info
            if self.latency > latency_threshold:
                ...
                if self.Debug:
                    self.log_file.write("skip events: skip_time_frame, play_frame, new_download_frame, ADD_frame" + str(
                        self.skip_time_frame) + " " + str(self.play_time_counter) + " " + str(
                        self.video_chunk_counter) + " " + str(self.add_frame) + "\n")
            else:
                self.video_chunk_counter += 1
            '''print("%.4f"%self.time ,
                      "  cdn %4f"%cdn_rebuf_time,
        ...
        # If not buffering
            ...
            if self.latency > latency_threshold:
                ...
                if self.Debug:
                    self.log_file.write(
                        "skip events: skip_download_frame, play_frame, new_download_frame, ADD_frame" + str(
                            self.skip_time_frame) + " " + str(self.play_time_counter) + " " + str(
                            self.video_chunk_counter) + " " + str(self.add_frame) + "\n")
            else:
                self.video_chunk_counter += 1
        ...
        if end_of_video:
            ...
            self.video_size = {}  # in bytes
            self.cdn_d_arrive_time = []
            self.gop_time_len = {}
            self.gop_flag = {}
            temp_delay_counter = self.delay_cnt
            self.delay_cnt = 0
            for bitrate in range(BITRATE_LEVELS):
                self.video_size[bitrate] = []
                #self.cdn_arrive_time[bitrate] = []
            ...
            return [self.time,  # physical time
                    ...
                    True,
                    temp_delay_counter]  # Is the end of video
