...
def test(user_id, color):
    ...
    VIDEO_TRACE = 'Fengtimo_2018_11_3'
    ...
    time2 = 0
    get_frame_cnt = 0
    choose_cnt = [] # testing varabile
    QoE_cnt = [] # testing varabile
    choose_cnt1 = [0,0,0,0]
    while True:
        ...
        get_frame_cnt += 1
        #print('get fm cnter = ', get_frame_cnt)
        time,time_interval, send_data_size, chunk_len,\
               rebuf, buffer_size, play_time_len,end_delay,\
                cdn_newest_id, download_id, cdn_has_frame,skip_frame_time_len, decision_flag,\
                buffer_flag, cdn_flag, skip_flag,end_of_video, delay_counter = net_env.get_video_frame(bit_rate,target_buffer, latency_limit,user_id)
        ...
        if decision_flag or end_of_video:
            ...
            bit_rate, target_buffer, latency_limit = abr.run(time,
                    ...
                    cdn_has_frame,
                    abr_init,
                    net_env.D_Trace.last_bitrate)

            choose_cnt.append(bit_rate)
            choose_cnt1[bit_rate] +=1
            timestamp_end = tm.time()
            call_time_sum += timestamp_end - timestamp_start # here is the only position where "call_time_sum" increases 
            # -------------------- End --------------------------------
            
        if end_of_video:
            print("network traceID, network_reward, avg_running_time, count", trace_count, reward_all, call_time_sum/cnt, cnt, delay_counter, get_frame_cnt)
            for bit_rate in range(4):
                print(bit_rate, 'counter =', choose_cnt1[bit_rate])
                choose_cnt1[bit_rate] = 0
            # plt.scatter(x=range(len(choose_cnt)),y=choose_cnt,marker='.')
            # plt.xlabel('I_frame_NO.')
            # plt.ylabel('bitrate')
            # plt.show()# testing plot
            QoE_cnt.append(reward_all)
